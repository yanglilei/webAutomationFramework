...
class DdddOcr(object):
    def __init__(self, ocr: bool = True, det: bool = False, old: bool = False, beta: bool = False,
        ...
        if det:
            ocr = False
            self.__graph_path = os.path.join(SysPathUtils.get_config_file_dir(), 'common_det.onnx')
            self.__charset = []
        if ocr:
            if not beta:
                self.__graph_path = os.path.join(SysPathUtils.get_config_file_dir(), 'common_old.onnx')
                ...
                self.__graph_path = os.path.join(SysPathUtils.get_config_file_dir(), 'common.onnx')
...
class MyDdddOcr:
    @classmethod
    def extract_verify_code_from_img(cls, captcha_img_path) -> str:
        with open(captcha_img_path, 'rb') as f:
            img_bytes = f.read()
        return cls.extract_verify_code_from_bytes(img_bytes)

    @classmethod
    def extract_verify_code_from_bytes(cls, captcha_img_bytes) -> str:
