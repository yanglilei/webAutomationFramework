...
from src.frame.common.playwright_stealth.case_insensitive_dict import CaseInsensitiveDict
from src.frame.common.playwright_stealth.context_managers import AsyncWrappingContextManager, SyncWrappingContextManager
from src.utils.sys_path_utils import SysPathUtils


def from_file(name) -> str:
    # return (Path(__file__).parent / "js" / name).read_text()
    return Path(SysPathUtils.get_config_file_dir(), "playwright_stealth_js", name).read_text()
