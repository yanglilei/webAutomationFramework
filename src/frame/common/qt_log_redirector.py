...
@singleton
class QtLogRedirectorV3(QObject):
    """协程 + Qt 日志类改造方案：日志自动携带协程专属用户名，协程启动时调用set_current_user方法"""
    signal = pyqtSignal(str)
    ...
    # ✅ 核心修改：用协程上下文变量替代线程本地存储
    # 定义上下文变量，默认值为 "FRAMEWORK"
    _username_ctx = contextvars.ContextVar("username", default="FRAMEWORK")

    # ✅ 改造：设置当前协程的用户名（协程内调用，支持异步/同步场景）
    def set_current_user(self, username: str):
        """为当前执行协程绑定用户名，该协程后续所有日志自动携带此用户名
        支持：1.异步协程内调用 2.同步线程内调用（兼容原有逻辑）
        """
        self._username_ctx.set(username)

    # ✅ 改造：获取当前协程的用户名（内部日志过滤使用）
    def get_current_user(self) -> str:
        """获取当前协程/线程绑定的用户名，未设置则返回默认值"""
        try:
            # 优先从协程上下文获取（协程场景）
            return self._username_ctx.get()
        except RuntimeError:
            # 兼容：无协程上下文时（纯线程场景），返回默认值
            return "FRAMEWORK"

    # ✅ 保持过滤器逻辑不变（内部自动调用改造后的get_current_user）
    class UserContextFilter(logging.Filter):
        def __init__(self, qt_logger):
            super().__init__()
            self.qt_logger = qt_logger
    ...
    def create_logger(
        ...
        if not has_root_file_handler:
            # 文件日志格式 - 保留【%(username)s】占位符，自动显示协程用户名
            root_formatter = logging.Formatter(
                '[%(asctime)s][%(levelname)s][%(username)s]%(module)s:%(lineno)s-%(message)s',
                "%Y-%m-%d %H:%M:%S"
            )
        ...
        if not has_business_stream_handler:
            # UI日志格式 - 保留【%(username)s】占位符，自动显示协程用户名
            stream_formatter = logging.Formatter(
                '[%(asctime)s][%(levelname)s][%(username)s]%(module)s:%(lineno)s-%(message)s',
                "%Y-%m-%d %H:%M:%S"
            )
