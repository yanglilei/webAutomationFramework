...
def parse(url: str, bookmarks: bool) -> tuple[str, str, list] | tuple[None, None, None]: # 解析资源，获取资源下载链接
    try:
...
def parse_and_copy() -> None: # 解析并复制链接
    ...
    for url in urls:
        resource_url = parse(url, False)[0]
        if not resource_url:
...
def download() -> None: # 下载资源文件
    ...
    urls = [{ "url": line.strip(), "invalid": False } for line in url_text.get("1.0", tk.END).splitlines() if line.strip()] # 获取所有非空行
    ...
    for url in urls:
        resource_url, title, chapters = parse(url["url"], bookmark_var.get())
        if not resource_url:
            url["invalid"] = True
            continue
    ...
    if any(url["invalid"] for url in urls):
        messagebox.showwarning("警告", "以下 “行” 无法解析：\n" + "\n".join([url["url"] for url in urls if url["invalid"]])) # 显示警告对话框

    if all(url["invalid"] for url in urls):
...
def add_bookmarks(pdf_path: str, chapters: list) -> None: # 给 PDF 添加书签
    try:
        if not chapters:
            return
...
bookmark_checkbox = ttk.Checkbutton(container_frame, text="添加书签", variable=bookmark_var)
