...
class Task:
    ...
    def __init__(self, driver, user_manager, global_config, user_config, task_config, logger):
        ...
        self.task_uuid = f'{self.batch_no}_{self.task_tmpl_id}_{str(uuid.uuid4()).replace("-","")}'  # 任务ID=uuid4_[任务模板ID]
        self.nodes: Dict[int, BaseNode] = {}  # 节点注册表：key=node_id, value=BaseTaskNode实例
        self.current_node_id: Optional[int] = None  # 当前执行的节点ID
    ...
    def run(self, reset_context: bool = True) -> bool:
        ...
        if not self.start_node_id or self.start_node_id not in self.nodes:
            raise ValueError("起始节点未配置或不存在")

        self.logger.info(
            f"===== 启动【{self.task_config.get('batch_info').get('project_name')} - {self.task_tmpl.get('name')}】任务 =====")
        max_task_relogin_times = self.task_config.get("task_tmpl_config", {}).get("relogin_config", {}).get(
            "max_task_relogin_times",
            3)  # 任务整体最大重登次数
        self.current_node_id = self.start_node_id  # 当前执行节点ID
        task_relogin_count = 0  # 任务整体重登次数（从头执行的次数）
        is_success = True
        try:
