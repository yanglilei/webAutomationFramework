...
class UIScheduledTask(BaseTableWidget):
    ...
    def get_headers(self) -> List[TableHeader]:
        return [
            ...
            TableHeader('上次执行时间', 'last_run_time'),
            TableHeader('下次执行时间', 'next_run_time'),
            TableHeader("Cron表达式", 'cron_expression'),
    ...
    def get_records(self, condition: dict, page=1, page_size=0) -> Tuple[List[dict], int]:
        if page_size > 0:
            return self.scheduled_task_dao.get_page_data(page, page_size,
                                                     project_name=condition.get("project_name"),
                                                     task_tmpl_name=condition.get("task_tmpl_name"))
        else:
            records = self.scheduled_task_dao.get_all()
    ...
    #         batch_id = row.get("id")
    #         # 发送信号给TaskManager
    #         # self.scheduled_task_dao.update_by_id(batch_id, {"execute_status": 4})
    ...
    def get_add_one_callable(self) -> Callable:
        return self.scheduled_task_dao.add_one

    def get_update_callable(self) -> Callable:
        return self.scheduled_task_dao.update_by_id

    def get_delete_data_callable(self) -> Callable:
        return self.scheduled_task_dao.delete_by_ids
    ...
    def get_query_fields(self) -> List[QueryField]:
        return [QueryField('text', '项目名称','project_name'), QueryField('text', '任务模版名称','task_tmpl_name')]
