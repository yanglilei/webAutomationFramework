...
class HXJYWLoginTaskNode(BaseLoginTaskNode):
        ...
        if btn_trigger_login_dialogue and await btn_trigger_login_dialogue.is_enabled() and await btn_trigger_login_dialogue.is_displayed():
            ...
            if login_dialogue:
                if basic.is_phone_no(self.username):
                    ret = await self._common_login(self.username, self.password,
    ...
    async def _common_login(self, username, password, login_tab_xpath, username_input_xpath, password_input_xpath,
                      captcha_img_xpath, verify_code_input_xpath, login_btn_xpath, project_code_xpath="",
                      retry_count=20):
        ret = True, "登录成功"
        tab = await self.get_elem_with_wait_by_xpath(10, login_tab_xpath)
        await tab.click()
        username_input = await self.get_elem_with_wait_by_xpath(10, username_input_xpath)
        if not username_input:
            return False, "用户名输入框找不到"

        password_input = await self.get_elem_with_wait_by_xpath(10, password_input_xpath)
        if not password_input:
            return False, "密码输入框找不到"

        login_btn = await self.get_elem_with_wait_by_xpath(10, login_btn_xpath)
        if not login_btn:
        ...
        project_code_input = await self.get_elem_with_wait_by_xpath(10, project_code_xpath)
        if project_code_input:
            await project_code_input.fill(self.project_code)

        while retry_count > 0:
            retry_count -= 1
            captcha_img_elem = await self.get_elem_with_wait_by_xpath(10, captcha_img_xpath)
            if captcha_img_elem:
            ...
            fail_tips = await self.get_elem_with_wait_by_xpath(3, xpath)
            if not fail_tips:
                # 登录成功
                break
            else:
                fail_desc = await fail_tips.text_content()
                # 非常重要，等待提示框消失，否则下次循环的时候，会重新获取到旧的提示框！！
                await self.wait_for_disappeared_by_xpath(10, xpath)
