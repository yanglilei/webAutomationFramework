...
class QZJXJYLogin(BaseLoginTaskNode):
    """
    泉州继续教育登录
    https://www.qzjxjy.com/
    """
    slider_img_path: str = ""
    ...
    async def _start_login(self):
        ret = True, "登录成功"
        username_input = await self.get_elem_with_wait_by_xpath(20, "(//input[@class='ipt'])[1]")
        if not username_input:
            ret = False, "用户名输入框找不到"
        else:
            password_input = await self.get_elem_by_xpath("(//input[@class='ipt'])[2]")
            if not password_input:
                ...
                agree_cb = await self.get_elem_by_xpath("//div[@class='bd']//div[@class='m-protocol-login']//span[@class='el-checkbox__inner']")
                if agree_cb:
                    await agree_cb.click()

                await asyncio.sleep(random.randint(1, 2))
                login_btn = await self.get_elem_by_xpath("//div[@class='bd']//button")
                if not login_btn:
                    ret = False, "登录按钮找不到到"
                else:
                    try:
                        ...
                        fail_tips = await self.get_elem_with_wait_by_xpath(2, "//p[@class='el-message__content']")
                        if fail_tips:
                            # 登录失败
    ...
    async def _slider_verify(self):
        ...
        while True:
            ...
            try:
                background_img_elem = await self.get_elem_with_wait_by_xpath(10, "//img[@id='tianai-captcha-slider-bg-img']")
                if not background_img_elem:
                    self.logger.error("滑块验证码，背景图片找不到")
                    await self.refresh()
                    await asyncio.sleep(2)
                    continue

                base64_to_image(await background_img_elem.get_attribute("src"), self.background_img_path)
                btn_slider = await self.get_elem_with_wait_by_xpath(10, "//div[@id='tianai-captcha-slider-move-btn']")
                ...
                box = await background_img_elem.bounding_box()
                ratio = 600 / box['width']
                # 实际移动的距离=该方法计算出的距离/缩放比例 - 滑块的起始距离
                # move_x = int(x / ratio - 30 / ratio * 2.4)
                move_x = int(x / ratio - 10 / ratio)
                await SliderVerifyUtils.move_slider_slowly_pw_version(move_x, btn_slider, self.get_current_page(), 0.5)
                await asyncio.sleep(2)

                background_img_elem = await self.get_elem_by_xpath("//img[@id='tianai-captcha-slider-bg-img']")
                if background_img_elem:
                    # 验证失败，刷新验证码
