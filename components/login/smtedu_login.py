...
class SMTEDULogin(BaseLoginTaskNode):
    ...
    def _start_login(self):
        ret = True, "登录成功"
        username_input = self.get_elem_with_wait_by_xpath(20, "//input[@id='username']")
            ...
            if not password_input:
                ...
                if not login_btn:
                    ...
                    try:
                        ...
                        if not self._slider_verify():
                            raise BusinessException("滑块验证失败")

                        fail_tips = self.get_elem_with_wait_by_xpath(2, "//p[@id='loginFormError']")
                        if fail_tips:
                            # 登录失败
                            ret = False, fail_tips.text
                        self.handle_week_password_alert()
                        self.handle_accept_agreement()
                        # 获得认证串
                        # self.auth_str = self.get_auth_str()
        return ret

    def _slider_verify(self):
        """
        滑块验证
        :return:
        """
        ...
        while True:
            ...
            # 由于点击了登录按钮会加载新的iframe到页面中，必须要等待页面加载完成，再执行获取元素的动作，所以这里不能用直接用get_elem_with_wait_by_xpath会抛出ContextLostError错误，drissionpage非常垃圾的地方
            self.wait_doc_loaded(2)
            captcha_xpath = "//div[@id='tcaptcha_transform_dy']//iframe[@id='tcaptcha_iframe_dy']"
            captcha_iframe: ChromiumFrame = self.get_elem_with_wait_by_xpath(10, captcha_xpath)
            back_ground_img_elem = self.get_elem_by_xpath("//div[@id='slideBg']", iframe=captcha_iframe)
            try:
                style_str = back_ground_img_elem.attr("style")
                ...
                btn_sliders = captcha_iframe.eles(".tc-fg-item")
                btn_slider = None
                for i in range(len(btn_sliders)):
                    btn_slider = btn_sliders[i]
                    size = btn_slider.rect.size
                    if 49 <= int(size[0]) <= 51:
                        break
                if not btn_slider:
                    captcha_iframe.ele("#e_reload").click()
                    time.sleep(2)
                    continue
                ...
                # iframe = self.get_frame(iframe_name="tcaptcha_iframe_dy")
                # iframe = self.get_frame(iframe_name="https://turing.captcha.qcloud.com")
                # btn_sliders = iframe.evaluate('document.querySelectorAll(".tc-fg-item");')
                # btn_sliders = self.execute_js('document.querySelectorAll(".tc-fg-item");', iframe=iframe)
                ...
                move_x = int(x / 2.4 - 30)
                SliderVerifyUtils.move_slider_slowly_dp_version(move_x, btn_slider, captcha_iframe)
                # if self.wait_for_disappeared_by_xpath(3, captcha_xpath):
                #     # 验证成功
                #     LOG.info(f"滑块验证成功")
                #     ret = True
                #     break
                # else:
                #     self.get_elem("#e_reload", captcha_iframe).click()
                #     time.sleep(2)
                # 跳转失败，确认二维码是否还在，若还在，则登录失败
                back_ground_img_elem = self.get_elem_with_wait_by_xpath(2, "//div[@id='slideBg']",
                ...
                if back_ground_img_elem:
                    ...
                    ret = True
                    break
                # if not self.get_current_page().wait.url_change(self.login_url, False, 3):
                #     # 跳转失败，确认二维码是否还在，若还在，则登录失败
                #     back_ground_img_elem = self.get_elem_with_wait_by_xpath(2, "//div[@id='slideBg']",
                #                                                             iframe=captcha_iframe)
                #     if back_ground_img_elem:
    ...
    def handle_week_password_alert(self):
        alert = self.get_elem_with_wait_by_xpath(3, "//div[@id='modify__tips_wrapper']")
        if alert:
    ...
    def handle_accept_agreement(self):
        accept_btn = self.get_elem_with_wait_by_xpath(3, "//button[@id='gotologon_sdk']")
        if accept_btn:
            try:
                accept_btn.click()
            except:
