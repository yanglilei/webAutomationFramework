...
class YanxiuScreenShot(BaseEnterCourseTaskNode):
    # 请求头
    ...
    async def enter_course(self) -> Tuple[bool, str]:
        if await self._is_finished(self.modules, self.project_id, self.tool_id, self.segment_id, self.subject_id):
            # 截图
            self.logger.info(f"{self.segment_name}{self.subject_name}-已学完，准备截图！")
            await self._screenshot()
            return False, f"{self.segment_name}{self.subject_name}-已学完"
        else:
            self.logger.info(f"未学完！")
            return True, ""

    async def _screenshot(self):
        save_dir = self.node_config.get("node_params", {}).get("save_dir")
        file_name = Path(save_dir, f"{self.segment_name}{self.subject_name}", f"{self.username}.png")
        await self.screenshot(file_name)
        self.logger.info(f"截图成功，保存路径：{file_name}")
        self.user_manager.update_record_by_username(self.username, {5: str(file_name)})
