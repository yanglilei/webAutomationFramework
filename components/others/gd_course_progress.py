...
class GDEnterCourse(BaseEnterCourseTaskNode):
    ...
    async def prepare_before_first_enter_course(self) -> Tuple[bool, str]:
        ...
        # 进入章节页面
        try:
            await self.get_study_progress()
            return True, ""
        except:
            self.logger.exception("❌获取学习进度失败：")
            return False, "没有获取到学习进度"
    ...
    async def get_study_progress(self):
        study_record_tab_elem = await self.get_elem_with_wait_by_xpath(5, "//a[@title='学习记录']", visible=False)
        if study_record_tab_elem:
            await self.js_click(study_record_tab_elem)
            # study_record_tab_elem.click()
        else:
            self.logger.error("没有找到“学习记录”的标签，网络问题或者页面有变动")
            raise BusinessException("没有找到“学习记录”的标签，网络问题或者页面有变动")

        await asyncio.sleep(2)
        # 切换iframe
        iframe = self.get_current_page().frame_locator("iframe").nth(0)

        # 获取任务点完成数量
        job_finished_info = await self.get_elems_with_wait_by_xpath(10, "//span[@id='jobfinish'] | //span[@id='jobPublish']", iframe=iframe)
        if not job_finished_info or len(job_finished_info) != 2:
            self.logger.error("没有获取到任务点完成数量，网络问题或者页面有变动")
            raise BusinessException("没有获取到任务点完成数量，网络问题或者页面有变动")

        finished_count = await job_finished_info[0].text_content()
        total_count = await job_finished_info[1].text_content()

        # 获取出勤次数
        attendance_count_elem = await self.get_elem_with_wait_by_xpath(10, "//span[@id='attendanceCount']", iframe=iframe)
        if not attendance_count_elem:
            self.logger.error("没有获取到“出勤次数”，网络问题或者页面有变动")
            raise BusinessException("没有获取到“出勤次数”，网络问题或者页面有变动")

        # 获取缺勤次数
        absence_count_elem = await self.get_elem_with_wait_by_xpath(10, "//span[@id='realAbsenceCount']", iframe=iframe)
        if not attendance_count_elem:
            self.logger.error("没有获取到“缺勤次数”，网络问题或者页面有变动")
            raise BusinessException("没有获取到“缺勤次数”，网络问题或者页面有变动")

        discussion_elems = await self.get_elems_with_wait_by_xpath(10, "(//div[@class='list fl'])[2]//span[@class='strong']", iframe=iframe)
        if not discussion_elems or len(discussion_elems) != 3:
            self.logger.error("没有获取到“讨论”的次数，网络问题或者页面有变动")
            raise BusinessException("没有获取到“讨论”的次数，网络问题或者页面有变动")

        attendance_count = await attendance_count_elem.text_content()
        absence_count = await absence_count_elem.text_content()
        send_count = await discussion_elems[0].text_content()
        reply_count = await discussion_elems[1].text_content()
        like_count = await discussion_elems[2].text_content()

        live_info = await self.get_elems_with_wait_by_xpath(10, "//span[@id='liveJobFinish'] | //span[@id='liveJobPublish'] | //span[@id='livingTime'] | //span[@id='playbackTime']", iframe=iframe)
        if live_info and len(live_info) == 4:
            live_finished_count = await live_info[0].text_content()
            live_total_count = await live_info[1].text_content()
            live_living_time = await live_info[2].text_content()
            live_playback_time = await live_info[3].text_content()
        else:
            self.logger.error("没有获取到直播信息，网络问题或者页面有变动")
            raise BusinessException("没有获取到直播信息，网络问题或者页面有变动")


        desc = f"完成进度：{finished_count}/{total_count}，出勤次数：{attendance_count}，缺勤次数：{absence_count}，发帖次数：{send_count}，回帖次数：{reply_count}，获赞数：{like_count}。直播进度：{live_finished_count}/{live_total_count}个，观看时长：{live_living_time}分钟，回放时长：{live_playback_time}分钟"
        self.user_manager.update_record_by_username(self.username, {5: desc})
