...
class HBTEEnterCourseTaskNode(BaseEnterCourseTaskNode):
    """
    河北教师教育网进入课程
    """
    # 计划ID
    ...
    async def prepare_before_first_enter_course(self) -> Tuple[bool, str]:
        ...
        await self.switch_subject_to_20()
        # await self.js_click(tab_new_class)
        return True, ""
    ...
    async def switch_subject_to_20(self):
        tab_new_class = await self.get_elem_with_wait_by_xpath(20,
                                                               "//ul[@class='selectItemSecond']//li[.//text()='20学时培训']")
        await tab_new_class.click()
    ...
    async def _get_pub_first_course(self) -> Locator:
        ...
        courses = await self.get_elems_with_wait_by_xpath(20, xpath, False)
        if courses:
